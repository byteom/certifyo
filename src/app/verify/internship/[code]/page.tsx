...
function VerifyInternshipContent() {
  ...
  const verifyInternship = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Internship certificate not found. Please check the verification code."
      );
    } finally {
      setLoading(false);
    }
  ...
  if (error || !internship) {
    return (
      ...
      >
        <div
          className={`$${isDarkMode ? "bg-gray-800" : "bg-white"} p-8 rounded-lg shadow-xl text-center max-w-md mx-4`}
        >
          <ExternalLink
            className={`h-16 w-16 mx-auto mb-4 ${isDarkMode ? "text-red-400" : "text-red-500"}`}
          />
          <h2
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? "bg-gray-900" : "bg-gray-50"} py-12`}>
      <div className="max-w-4xl mx-auto px-4">
        <div
          className={`$${isDarkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-xl overflow-hidden`}
        >
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-8 text-white text-center">
            <CheckCircle className="h-16 w-16 mx-auto mb-4" />
            <h1 className="text-3xl font-bold mb-2">Internship Certificate Verified</h1>
            <p className="text-indigo-100">
              Verification Code: {internship.verification_code}
            </p>
          </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center">
                    <Award
                      className={`h-5 w-5 mr-3 ${isDarkMode ? "text-indigo-400" : "text-indigo-600"}`}
                    />
                    <div>
                      <p
                    ...
                    <div className="border-t border-gray-200 pt-4 mt-4">
                      ...
                      {internship.company_address && (
                        <div className="flex items-center mb-2">
                          <MapPin
                            className={`h-4 w-4 mr-2 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
                          />
                          <span
