...
const ThreeDBackground = () => {
  ...
  useFrame((state) => {
    ...
    groupRef.current.rotation.y += 0.001; // Slow rotation for background effect
  });

  return (
    <>
      {/* Create some 3D objects for the background */}
      <group ref={groupRef}>
...
export default function ThreeScene() {
  return (
    <Canvas
      ...
      onCreated={({ gl }) => {
        gl.setClearAlpha(0); // Make the background transparent
      }}
    >
      <ambientLight intensity={0.3} />
      <pointLight position={[10, 10, 10]} intensity={0.8} />
      <ThreeDBackground /> {/* Add the 3D background */}
      <FloatingSubjects /> {/* Floating subjects */}
